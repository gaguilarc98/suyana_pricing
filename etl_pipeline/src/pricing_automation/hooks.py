...
class ProjectHooks:
    ...
    def _setup_aws(self):
        if os.environ.get("PRICING_AUTOMATION_SKIP_AWS_HOOK") == "1":
            logger.info("AWS: skipping setup because PRICING_AUTOMATION_SKIP_AWS_HOOK=1")
            return
    ...
    def _try_instance_metadata(self):
        try:
            provider = InstanceMetadataProvider(
                iam_role_fetcher=InstanceMetadataFetcher(timeout=60, num_attempts=2)
            )
